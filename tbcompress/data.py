...
class TablebaseDataset(Dataset):
    ...
    def _load_positions(self):
        ...
        print(
            f"Generating complete position space for {material} ({piece_count} pieces)"
        )

        # Generate positions systematically to cover the entire space
        if (
            piece_count <= 3
        ):  # For very simple material balances, use brute force enumeration
            position_generator = self._exhaustively_enumerate_positions(material)
        elif (
            piece_count <= 5
        ):  # For moderate complexity, use systematic sampling with reflection/rotation equivalence
        ...
        print(
            f"Processed {positions_processed} positions, found {valid_positions} valid positions"
        )
    ...
    def _exhaustively_enumerate_positions(self, material):
        ...
        for wk_square in chess.SQUARES:
            ...
            for bk_square in chess.SQUARES:
                ...
                # More complex simple endgames with 3 pieces total - iterate all combinations
                elif (len(remaining_white) + len(remaining_black) == 1) and (
                    len(white_pieces) + len(black_pieces) == 3
                ):
                    ...
                    for piece_square in available_squares:
                        ...
                        for turn in [chess.WHITE, chess.BLACK]:
                            ...
                            board.set_piece_at(
                                piece_square, chess.Piece(piece_type, extra_piece_color)
                            )
                            board.turn = turn
            ...
            for wk_rank in range(4):  # 1-4 ranks
                ...
                for bk_file in range(8):  # a-h files
                    for bk_rank in range(8):  # 1-8 ranks
                        ...
                        if (
                            chess.square_distance(wk_square, bk_square) <= 1
                            or wk_square == bk_square
                        ):
                            continue
    ...
    def _place_remaining_pieces(
        self, base_board, remaining_white, remaining_black, piece_map, available_squares
    ):
        ...
        if remaining_white:
            ...
            if piece_type == chess.PAWN:
                ...
                valid_squares = [
                    sq for i, sq in enumerate(available_squares) if i % 2 == 0
                ]
            else:
    ...
    def _comprehensive_sampling(self, material):
        ...
        print(
            f"Generating comprehensive position sample for complex material: {material}"
        )
        ...
        while positions_generated < target_positions:
            ...
            if board is not None:
                ...
                if duplicate_count > 1000000:  # After 1M duplicates, refine strategy
                    duplicate_count = 0
                    print(
                        f"Refining position generation strategy after {positions_generated} positions"
                    )
                ...
                if positions_generated + duplicate_count > 5 * target_positions:
                    print(
                        f"Safety limit reached after generating {positions_generated} positions"
                    )
                    break
    ...
    def _generate_positions(self, material, target_count):
        ...
        while positions_yielded < target_count:
            ...
            if positions_generated >= 10 * target_count:
                print(
                    f"Warning: Generation limit reached. Only found {positions_yielded} positions"
                )
                break
